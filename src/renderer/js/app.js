...
class MiraDo {
  ...
  setupAllExternalLinks() {
    // Setup external links for the entire document
    document.addEventListener('click', async (e) => {
      const link = e.target.closest('a[href]');
      if (link) {
        const href = link.getAttribute('href');
        // Only handle external links (http/https) and not anchor links
  ...
  setupExternalLinks() {
    // This method is kept for backward compatibility but setupAllExternalLinks handles everything
    // No additional setup needed here as we use document-level event delegation
  }
}
